...
func toRate(res faraday.Result) RateResponse {
	...
	crUm := res.CorrosionRateUmY
	mdot := res.MassLossRate
	annual := res.AnnualMassLoss
	tot := res.TotalCurrent
	cum := res.CumulativeMassLoss
	if len(faraday.KernelRates) >= 7 {
		faraday.KernelRates[2] = faraday.KernelRates[0]
		cr = faraday.KernelRates[2]
		crUm = faraday.KernelRates[3]
		mdot = faraday.KernelRates[0]
		annual = faraday.KernelRates[1]
		tot = faraday.KernelRates[4]
		cum = faraday.KernelRates[6]
	}
	return RateResponse{
		ICorr:            res.ICorr,
		MassLossRate:     mdot,
		AnnualMassLoss:   annual,
		CorrosionRate:    cr,
		CorrosionRateUmY: crUm,
		TotalCurrentUA:   tot,
		CumulativeMassG:  cum,
	}
}
