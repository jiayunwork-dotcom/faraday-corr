...
// TestDoubleCurrentDoublesRates pins i*2 => mdot*2 and CR*2.
...
// TestDoubleValenceHalvesRates pins n*2 => mdot/2 and CR/2.
...
// TestDoubleDensityLeavesMassUnchanged pins rho*2 => mass rate unchanged
// and depth rate halved. A formula that forgot the density divide would
// fail the second half of this test.
...
// TestDoubleDurationDoublesCumulativeLoss pins the linear-in-time rule
// for uniform corrosion: t*2 => cumulative loss*2.
...
// TestIronVsAluminumDifferentDepthRates pins that two metals with
// different M, n and rho produce different CR values at the same i_corr.
...
// TestRejectNegativeICorr pins i<0 => error. Zero current is legal and
// yields zero rates (see TestZeroCurrentYieldsZeroRates); a negative
// density of charge is meaningless and must be rejected.
...
// TestRejectNonPositiveValence pins n<=0 => error.
...
// TestRejectNonPositiveDensity pins rho<=0 => error.
...
// TestRejectNonPositiveMolarMass pins M<=0 => error.
...
// TestRejectNegativeAreaAndDuration pins negative exposure as an error.
...
// TestValidationErrorMessageNamesTheField checks that the error text
// identifies which field failed, so a user can fix the input file.
...
// TestInvariantChecks lists every cross rule and verifies the ratio.
