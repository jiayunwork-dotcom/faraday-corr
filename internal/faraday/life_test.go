...
// TestYearsToPenetrate checks the linear allowance-consumption time.
...
// TestYearsToPenetrateZeroRate handles the degenerate zero-CR case.
...
// TestRemainingThickness checks the linear thickness subtraction.
func TestRemainingThickness(t *testing.T) {
	...
	// Fully consumed wall clamps to zero.
...
// TestPenetrationAfterYears checks the depth consumed over time.
...
// TestBuildSchedule checks the linear-in-time cumulative loss table.
