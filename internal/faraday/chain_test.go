...
// TestVerifyUnitChainCloses checks that the mass-loss path and the
// direct K path produce the same depth rate, and that the re-derived K
...
// TestVerifyUnitChainRejectsInvalidInput propagates validation errors.
...
// TestKDerivedMatchesPinned re-derives K from raw parts.
...
// TestUnitsUsedListsCoreUnits ensures the unit legend covers the kernel
// outputs.
